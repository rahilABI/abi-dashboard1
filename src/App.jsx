...
export default function App() {
  ...
  const handleEndDateChange = async (projectId, newEndDate) => {
    if (!currentUser?.isAdmin) {
      triggerToast("Permission denied: Only an Admin can change end dates.");
      return;
    }
    // Optimistic local update
    setProjects(projects.map(p => p.projectId === projectId ? { ...p, endDate: newEndDate } : p));
    
    // Upsert to Supabase
    const { error: updateErr } = await supabase.from('project_metrics').upsert({ ticket_id: projectId, end_date: newEndDate || null }, { onConflict: 'ticket_id' });
    if (updateErr) {
      console.error("Failed to update project end date:", updateErr);
      triggerToast(`Error updating end date: ${updateErr.message}`);
    } else {
      triggerToast(`End Date updated`);
    }
  };
                      ...
                      {currentUser?.isAdmin && (
                        <>
                          ...
                          <input 
                            ...
                            onChange={(e) => handleEndDateChange(project.projectId, e.target.value)}
            ...
            <div className={`mb-10 p-6 rounded-2xl border ${isDarkMode ? "bg-[#0d1217] border-[#00ffd5]/20 shadow-[inset_0_0_20px_rgba(0,255,213,0.05)]" : "bg-[#F8FAFC] border-[#DFE1E6]"}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6 pb-6 border-b border-dashed border-[#00ffd5]/20">
                <div>
                  ...
                  <input type="date" value={modalForm.startDate} disabled={!canEditStatusAndPublish(activeProject)} onChange={(e) => setModalForm({...modalForm, startDate: e.target.value})} className={`w-full border h-10 px-3 rounded-lg text-xs font-semibold focus:outline-none transition-all ${!canEditStatusAndPublish(activeProject) ? "cursor-not-allowed opacity-80" : ""} ${isDarkMode ? "bg-[#15191e] shadow-inner border border-[#2a343b] text-slate-200 focus:bg-[#0a0e12] focus:border-[#00ffd5] focus:ring-1 focus:ring-[#00ffd5]/50 text-white focus:border-[#00ffd5] focus:ring-1 focus:ring-[#00ffd5]/50" : "bg-white border-[#DFE1E6] text-slate-800"}`} />
                </div>
                <div>
